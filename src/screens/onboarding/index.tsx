...
export const OnboardingScreen: React.FC = () => {
    ...
    return (
        <>
            <OnboardingLayout>
                <Column padding={'16px'}>
                    <Bold18x27 textAlign={'center'}>
                        {ONBOARDING_ITEMS[selectedIndex].header}
                    </Bold18x27>
                </Column>
                {renderIndicators()}
                <Carousel
...
const OnboardingCarouselItem: React.FC<{ item: OnboardingItem; isFinalStep: boolean; isFirstStep: boolean; }> = ({ item, isFinalStep, isFirstStep }) => {
    ...
    return (
        <Column alignItems={'center'} justifyContent={'center'}>
            <Medium18x27 style={{ paddingLeft: 16, paddingRight: 16 }}>
                {!isFirstStep ? item.description : (
                    <React.Fragment>
                        <HighlightText fontColor={theme.palette.pink}>
                            DeFi 2.0 Wallet
                        </HighlightText>
                        {' that helps you'}
                        <HighlightText fontColor={theme.palette.blue}>
                            {' earn more '}
                        </HighlightText>
                        {' by '}
                        <HighlightText fontColor={theme.palette.yellow}>
                            doing less.
                        </HighlightText>
                    </React.Fragment>
                )}
            ...
            {
                isFinalStep ? (
                    <GetStartedButtonLayout onClick={onGetStartedClick} width={'calc(100% - 32px)'} height={'56px'} filled>
